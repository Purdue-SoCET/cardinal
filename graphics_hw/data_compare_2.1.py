...
class DCacheSimulator:
    """Tightly coupled Tag/Data array. Blocking Cache. No FIFOs or ROB."""
    ...
    def access_batch(self, texel_addrs):
        self.cycles += 1 # 1 base cycle to receive the TMU batch
        ...
        for addr in texel_addrs:
            ...
            if tag in self.sets[set_idx]:
                ...
                self.line_fills += 1
                batch_stall_cycles += MISS_PENALTY # Head-of-line blocking
                
                self.sets[set_idx].insert(0, tag)
                if len(self.sets[set_idx]) > ASSOC:
                    self.sets[set_idx].pop()
                    
        self.cycles += batch_stall_cycles

    def flush(self):
        pass # D-Cache is blocking, memory is always resolved inline
...
class TCacheSimulator:
    """Decoupled Non-Blocking Cache with Frag FIFO, MSHR, and ROB."""
    def __init__(self, name):
        self.name = name
        self.sets = [[] for _ in range(NUM_SETS)]
        self.seen_blocks = set()
        ...
        self.frag_fifo = []        # Size 2N (32)
        self.mshr = {}             # Maps block_addr -> ready_cycle. Size N (16)
        self.rob = []              # In-order completion queue. Size N (16)
        self.frag_size = 2 * N_SIZE
        self.mshr_rob_size = N_SIZE
    ...
    def tick(self):
        ...
        # 1. Clear memory fetches that have returned
        completed_mshr = [addr for addr, ready in self.mshr.items() if ready <= self.cycles]
        for addr in completed_mshr:
            del self.mshr[addr]
            
        # 2. Retire completed ROB entries (in-order)
        while self.rob and self.rob[0] <= self.cycles:
            self.rob.pop(0)
            
        # 3. Controller processes up to 4 tags per cycle from Frag FIFO
        processed = 0
        while processed < 4 and self.frag_fifo:
            if len(self.rob) >= self.mshr_rob_size:
                break # ROB full, backpressure stalls controller
            ...
            if tag in self.sets[set_idx]:
                ...
                # Miss Path
                if block_addr in self.mshr:
                    # Coalesced hit in MSHR
                    self.misses += 1
                    self.rob.append(self.mshr[block_addr])
                    self.frag_fifo.pop(0)
                    processed += 1
                else:
                    # True Miss
                    if len(self.mshr) >= self.mshr_rob_size:
                        break # MSHR full, backpressure stalls controller
                    ...
                    self.mshr[block_addr] = ready_cyc
                    self.rob.append(ready_cyc)
                    
                    # Allocate cache line immediately (evict LRU)
                    self.sets[set_idx].insert(0, tag)
                    if len(self.sets[set_idx]) > ASSOC:
                        self.sets[set_idx].pop()
    ...
    def access_batch(self, texel_addrs):
        ...
        # Wait if TMU injection would overflow Fragment FIFO
    ...
    def flush(self):
        """Drain remaining buffers at end of simulation."""
