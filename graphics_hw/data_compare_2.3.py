...
T-cache with Miss Request FIFO and ROB, using linear search to coalesce mem access and  decoupled tag/data, allows coalescing of misses within ROB depth N.
...
class TCacheSimulator:
    """
    T-Cache using a Linear Search approach:
    Scans the entire Miss Request FIFO to achieve perfect coalescing without an MSHR CAM.
    """
    def __init__(self, name):
        self.name = name
        self.sets = [[] for _ in range(NUM_SETS)]
    ...
    def tick(self):
        ...
        while processed < 4 and self.frag_fifo:
            ...
            # 2. Miss Path
                ...
                # Coalescing: Linear Search the FIFO
                found_ready_cyc = None
                for fifo_block, fifo_ready in self.miss_req_fifo:
                    if fifo_block == block_addr:
                        found_ready_cyc = fifo_ready
                        break
                       
                if found_ready_cyc is not None:
                    self.rob.append(found_ready_cyc)
                   
                # True Miss (Requires Memory Fetch)
                else:
                    if len(self.miss_req_fifo) >= self.fifo_size:
                        break # Miss FIFO full
