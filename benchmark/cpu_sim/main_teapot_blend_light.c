...
#include "../kernels/include/blend_light.h"
...
int main(int argc, char** argv) {
    ...
    blend_args->verts = pVerts;
    blend_args->num_verts = num_verts;
    blend_args->tris = tris;
    blend_args->num_tris = num_tris;

    blend_args->threeDVertTrans = tVerts;
    blend_args->camera = *camera_C;
    blend_args->sphere_center = *Oa;
    blend_args->light_pos = (vector_t){camera_C->x, camera_C->y, camera_C->z};

    blend_args->albedo = (vec4_t){1.0f, 0.2f, 0.2f, 1.0f};
    blend_args->ambient = (vector_t){0.20f, 0.04f, 0.04f};
    blend_args->kd = 0.95f;
    blend_args->ks = 0.03f;
    ...
    for(int tri = 0; tri < num_tris; tri++) {
        ...
        run_kernel(kernel_blend_light, grid_dim, block_dim, (void*)blend_args);
    }
