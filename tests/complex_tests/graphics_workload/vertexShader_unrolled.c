...
#endif
{
    ...
    // Square the components to compare magnitudes (avoids needing abs())
    float x2 = args->a_dist->x * args->a_dist->x;
    float y2 = args->a_dist->y * args->a_dist->y;
    float z2 = args->a_dist->z * args->a_dist->z;

    // Pick the axis with the smallest magnitude in a_dist
    if (x2 < y2 && x2 < z2) {
        selAxis[0] = 1.0;
    } else if (y2 < z2) {
        selAxis[1] = 1.0;
   ...
   //normalize(lcs[0 to 2])
    ...
    p1[0] += lcsInv[0*3 + 1] * p_tempAxis[1]; // k = 1
    p1[0] += lcsInv[0*3 + 2] * p_tempAxis[2]; // k = 2

    // j = 1
    p1[1] += lcsInv[1*3 + 0] * p_tempAxis[0]; // k = 0
    p1[1] += lcsInv[1*3 + 1] * p_tempAxis[1]; // k = 1
    p1[1] += lcsInv[1*3 + 2] * p_tempAxis[2]; // k = 2

    // j = 2
    p1[2] += lcsInv[2*3 + 0] * p_tempAxis[0]; // k = 0
    p1[2] += lcsInv[2*3 + 1] * p_tempAxis[1]; // k = 1
    ...
    // Unrolled j and k loops
    // j = 0
    p2[0] += rotMat[0*3 + 0] * p1[0]; 
    p2[0] += rotMat[0*3 + 1] * p1[1]; 
    p2[0] += rotMat[0*3 + 2] * p1[2];

    // j = 1
    p2[1] += rotMat[1*3 + 0] * p1[0]; // k = 0
    p2[1] += rotMat[1*3 + 1] * p1[1]; // k = 1
    p2[1] += rotMat[1*3 + 2] * p1[2]; // k = 2

    // j = 2
    p2[2] += rotMat[2*3 + 0] * p1[0]; // k = 0
    p2[2] += rotMat[2*3 + 1] * p1[1]; // k = 1
    ...
    p_world[0] += lcs[0*3 + 1] * p2[1]; // k = 1
    p_world[0] += lcs[0*3 + 2] * p2[2]; // k = 2
    args->threeDVertTrans[i].coords.x = p_world[0] + args->Oa->x;

    // j = 1
    p_world[1] += lcs[1*3 + 0] * p2[0]; // k = 0
    p_world[1] += lcs[1*3 + 1] * p2[1]; // k = 1
    p_world[1] += lcs[1*3 + 2] * p2[2]; // k = 2
    args->threeDVertTrans[i].coords.y = p_world[1] + args->Oa->y;

    // j = 2
    p_world[2] += lcs[2*3 + 0] * p2[0]; // k = 0
    p_world[2] += lcs[2*3 + 1] * p2[1]; // k = 1
