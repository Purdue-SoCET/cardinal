...
public:
    explicit ForwardingInterface(std::string name_set) // initializer list, names the FWIF
...
public:
    explicit LatchInterface(std::string name_set) // initializer list, names the LatchIF
        : name_internal(std::move(name_set)) {}
    ...
    const std::optional<T>& snoop() const { //forwarding in pipeline sense. non-destructive read-only payload
        return payload_out;
    }
