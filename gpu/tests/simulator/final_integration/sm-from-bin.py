...
def run_test(
    ...
    print(f"\nRunning pipeline: {n_instr} issue cycles startting from {START_PC}")
...
def run_ldst_test(
    ...
    p = build_pipeline(program_file, fmt=fmt, start_pc=START_PC, tb_size=1024)
...
def _parse_args():
    ...
    parser.add_argument(
        "--startpc", "-pc",
        default=0x1000,
        help="Input the start pc to initialize the program with."
    )
    parser.add_argument(
        "--quiet", "-q",
        action="store_true",
