...
def get_test_output_dir():
    """
    Get the test output directory with timestamp.
    Returns: /home/.../gpu/gpu/test_results/YYYY-MM-DD_HH-MM-SS/
    
    This path is absolute and relative to the project structure,
    so it works regardless of where the test is run from.
    """
    # Get the directory containing this test file
    test_file_path = Path(__file__).resolve()
    
    # Navigate to gpu/gpu/ directory (5 levels up from tests/simulator/backend/writeback/test_wb_buffer.py)
    # writeback -> backend -> simulator -> tests -> gpu/gpu
    gpu_gpu_dir = test_file_path.parent.parent.parent.parent.parent
    
    # Create timestamped subdirectory
    timestamp = datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
    output_dir = gpu_gpu_dir / "test_results" / timestamp
...
def test_buffer_overflow(harness: WritebackTestHarness):
    ...
    try:
        ...
        # To cause buffer overflow, we need multiple FSUs targeting the same register bank
        # This creates contention at the bank level, preventing simultaneous writebacks
        # and causing buffers to fill up
        
        latches = list(harness.wb_stage.behind_latches.keys())
        target_bank = 0  # All instructions target the same bank to create contention
        
        result.add_detail(f"Creating bank contention by targeting bank {target_bank} from multiple FSUs")
        
        # Get individual buffer capacities
        buffer_capacities = {name: buf.capacity + 1 for name, buf in buffer.buffers.items()}
        largest_capacity = max(buffer_capacities.values())
        
        # Set max_iterations based on largest buffer capacity and number of buffers
        max_iterations = largest_capacity * len(buffer.buffers) * 3
        
        result.add_detail(f"Buffer capacities: {buffer_capacities}")
        result.add_detail(f"Max iterations set to {max_iterations} (3x largest buffer capacity: {largest_capacity})")
        
        # Track individual buffer occupancy over time
        buffer_max_occupancy = {name: 0 for name in buffer.buffers.keys()}
        
        # Push multiple instructions from different FSUs to the same bank
        # This will cause buffers to fill since only one can writeback per cycle per bank
        for i in range(max_iterations):
            # Push from multiple FSUs in the same cycle
            for j, latch_name in enumerate(latches[:min(5, len(latches))]):
                instr = create_instruction(
                    fsu_name=harness.fsu_names[j % len(harness.fsu_names)],
                    target_bank=target_bank,  # Same bank for all - creates contention
                    issued_cycle=harness.current_cycle + i,
                    pc_value=(i * 10 + j) * 4
                )
                harness.push_to_latch(latch_name, instr)
                
                # Tick once per iteration
                harness.tick()
            
            # Track individual buffer occupancies
            for buf_name, buf in buffer.buffers.items():
                current_occupancy = len(buf)
                buffer_max_occupancy[buf_name] = max(buffer_max_occupancy[buf_name], current_occupancy)
            
            # Check if any buffer has achieved significant fill
            total_occupancy = sum(len(buf) for buf in buffer.buffers.values())
            if total_occupancy >= 10:
                result.add_detail(f"Significant buffer fill at iteration {i + 1}")
                break
        
        # Display final individual buffer statistics
        result.add_detail("\nIndividual buffer statistics:")
        overflow_detected = False
        for buf_name, buf in buffer.buffers.items():
            final_occupancy = len(buf)
            capacity = buffer_capacities[buf_name]
            max_occupancy = buffer_max_occupancy[buf_name]
            
            result.add_detail(f"  {buf_name}: {final_occupancy}/{capacity} (max: {max_occupancy}/{capacity})")
            
            # Check for overflow
            if final_occupancy > capacity or max_occupancy > capacity:
                overflow_detected = True
        
        # Overall result
        if overflow_detected:
            result.mark_failed(f"Buffer overflow detected - exceeded capacity")
        else:
            max_fill_pct = max((buffer_max_occupancy[name] / buffer_capacities[name] * 100) 
                              for name in buffer.buffers.keys())
            result.mark_passed(f"Buffers respected capacity limits (max fill: {max_fill_pct:.1f}%)")
...
def run_config_tests(buffer_config: WritebackBufferConfig, regfile_config: RegisterFileConfig, config_name: str, fsu_names: List[str], output_dir: Path) -> tuple:
    ...
    wb_stage = WritebackStage(
        wb_config=buffer_config,
        rf_config=regfile_config,
    ...
    # Export performance counters to provided output directory
    config_dir = output_dir / config_name.replace(' ', '_')
    config_dir.mkdir(parents=True, exist_ok=True)
    
    wb_stage.wb_buffer.export_perf_counts(directory=str(config_dir))
...
def main():
    ...
    regfile_config = RegisterFileConfig.get_default_config()
    buffer_sizes, fsu_priorities = WritebackBufferConfig.create_fsu_mappings(fsu_names)

    # Create single timestamped output directory for this test run
    output_dir = get_test_output_dir()
    output_dir.mkdir(parents=True, exist_ok=True)
    print(Colors.blue(f"\nTest results will be saved to: {output_dir}\n"))
    
    # Test Configuration 1: Default
    print(Colors.bold("\n[1/3] Testing Default Configuration..."))
    config_default = WritebackBufferConfig.get_default_config()
    config_default.validate_config(fsu_names)
    passed, total, results = run_config_tests(config_default, regfile_config, "Config_Default", fsu_names, output_dir)
    all_results.append(("Config_Default", passed, total, results))
    
    # Test Configuration 2: Type One
    print(Colors.bold("\n[2/3] Testing Type One Configuration..."))
    config_type_one = WritebackBufferConfig.get_config_type_one(buffer_sizes, fsu_priorities)
    config_type_one.validate_config(fsu_names)
    passed, total, results = run_config_tests(config_type_one, regfile_config, "Config_Type_One", fsu_names, output_dir)
    all_results.append(("Config_Type_One", passed, total, results))
    
    # Test Configuration 3: Type Two
    print(Colors.bold("\n[3/3] Testing Type Two Configuration..."))
    config_type_two = WritebackBufferConfig.get_config_type_two(fsu_priorities)
    config_type_two.validate_config(fsu_names)
    passed, total, results = run_config_tests(config_type_two, regfile_config, "Config_Type_Two", fsu_names, output_dir)
    ...
    # Save the captured output to the same timestamped directory
    output_file = output_dir / "wb_buffer_test_output.ansi"
